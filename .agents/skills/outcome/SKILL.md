...
description: >
  Records what happened to a job application (interview invitation, status change, final hired/rejected outcome) and archives the materials.
  Triggers on: outcome, /outcome, record outcome, update application status, application resolved
---
...
- `job_search_tracker.csv` - the status column that the scraper and rank workflows use for dedup and exclusion
- `documents/applications/<company>_<role>/` - the per-application archive (posting, submitted drafts, `outcome.md`) that the setup workflow mines to calibrate `04-job-evaluation.md` and surface STAR candidates

Outcome writes the data; setup workflow interprets it. This command never edits the evaluation framework or profile files itself.
...
Create or update `documents/applications/<company>_<role>/`. All content here is personal data - the folder is already gitignored (`documents/applications/**`).

1. **`cv_draft.tex` and `cover_letter.tex`** - copy the submitted files. Locate them via the tracker row's `cv_file`/`cover_letter_file` columns; if those are empty, look for `cv/main_<company>.tex` and `cover_letters/cover_<company>_*.tex`. If a file already exists in the archive, leave it - the archived version is what was actually submitted. If no draft files exist, skip with a note.
2. **`job_posting.md`** - if it already exists, leave it. Otherwise try WebFetch on the tracker row's `source` URL and save the posting text. If the URL is dead, ask the user to paste the posting, or write a stub noting the posting is unavailable. **Never reconstruct a posting from memory.**
3. **`outcome.md`** - write or update it in exactly the format documented in `documents/README.md`, so setup workflow parses it without special cases:
...
## Notes
<feedback received, what to do differently, signals about what they valued - appended per update with a date, never overwritten>
```

Update rules: tick stage checkboxes as they are reached (add the date in parentheses), append dated entries to Notes, and only change `Status` from `in_progress` to a final value on resolution. Re-running on the same application is idempotent.
...
- If 3 or more are resolved (or 2+ share a pattern - same role type rejected twice, same sector going silent), suggest:
  > "You now have <N> resolved applications on record. Run the setup workflow (Path A) to fold them into your evaluation framework - it calibrates fit scoring from what actually got interviews, and mines your interview feedback for STAR examples."
- Do **not** write anything into `04-job-evaluation.md` or other skill files yourself. The setup workflow owns that merge.
...
> - Archived: <which files were copied/fetched or skipped>
> - Tracker: status → <new status>

If the update recorded an upcoming or newly scheduled interview stage, also suggest:

> "Interview coming up? Run the interview prep workflow (`interview prep <company>`) to build a prep pack for that stage from this application's archive."
